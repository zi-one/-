...
<div style="margin: 20px auto;">
    <form method="get" action="{{ url('teacher/studentList/'.$id) }}">
            ...
            <tbody>
                ...
                @foreach($student as $s)
                     <tr>
                       ...
                       <td>
                           @if($s->edit_status==1)
                           <input disabled="disabled" type="text" name="s_cj" cj-id="{{$s->sc_id}}" value="{{$s->c_cj}}"/>
                           @else
 ...
 function submit_cj(){
     ...
     $('input[name="s_cj"]').each(function(i,v){
         ...
         if(r) {
             ...
             $.post('/teacher/submitCj', {"ids": ids, "cj": cj, "_token": "{{csrf_token()}}"}, function (data) {
                 if (data.status == 0) {
                     alert(data.message);
                 } else {
                     alert(data.message);
  ...
  function save_cj(){
        ...
        $('input[name="s_cj"]').each(function(i,v){
           ...
           $.post('/teacher/saveCj',{"ids":ids,"cj":cj,"_token":"{{csrf_token()}}"},function(data){
               if(data.status==0){
                   alert(data.message);
               }else{
                   alert(data.message);
